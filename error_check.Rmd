...
```{r}
fan_duel <- read_csv("~/Desktop/week1_data.csv") %>% 
  mutate(Name = str_c(`First Name`, `Last Name`, sep = " ")) %>% 
  mutate(position = case_when(
    Position == "D" ~ "DST",
    TRUE ~ as.character(Position)
  )) %>% 
  select(-c(`Sunday Million`, `H2H`, `First Name`, `Last Name`, Position)) %>% 
  filter(Name != "Miles Sanders")
...
```{r sim_time}
sim_lu <- map_df(1:100, generate_lineup) %>%
...
```{r error_check}
x <- psel(lu_df, low(lineup_sd) * high(lineup_pts)) %>%
  left_join(sim_lu, by='lineup') %>%
  group_by(lineup) %>%
  arrange(lineup_pts, position, desc(Salary)) %>%
  select(lineup, lineup_pts, lineup_sd, Name, team, position, pts_pred, sd_pts, Salary) %>%
